...
$PAGE->set_title(get_string('notification_reminder_subject','local_variatives'));
$PAGE->set_heading( get_string('notification_reminder_subject','local_variatives'));
...
// echo "<pre>";print_r($varblock);echo "</pre>";exit();


echo "<h2>".get_string('notification_reminder_subject','local_variatives').": {$varblock->varblockname}</h2>";
...
foreach($list as $row){
    ...
    $fullmessagehtml=str_replace(
            ...
            get_string('notification_reminder_body','local_variatives')
    );
    $smallmessage=str_replace(
            ...
            get_string('notification_reminder_body_small','local_variatives')
    );
    ...
    echo " {$row->userid} {$row->userlastname} {$row->userfirstname};<br>\n";
}
