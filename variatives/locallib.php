...
function variatives_group_list() {
    ...
    $sql = "SELECT var_group.*, cohort.name AS cohortname, var_department.vardepartmentname,
         ...
         ORDER BY var_department.vardepartmentname, var_group.vargroupyear DESC";
    // echo $sql;
...
function variatives_blockgroup_list($varblockid) {
    ...
    $sql = "SELECT blockgroup.*, 
                   block.varblockname,
                   block.varblockminstudents,
                   block.varblockmaxstudents,
                   block.varblocktimestampfrom,
                   block.varblocktimestampto,
                   block.vargroupyear,
                   block.varformid,
                   block.varlevelid,
                   block.varblockisarchive,
          ...
          FROM {$CFG->prefix}var_blockgroup blockgroup
               INNER JOIN {$CFG->prefix}var_block block ON blockgroup.varblockid=block.id
...
function variatives_enroll_list($filter) {
    ...
    $users= & $assignment->assignment['users'];
    $list = array_values($DB->get_records_sql($sql, $param, $limitfrom, $limitnum));
    
    
    // echo '<pre>'; print_r($assignment->assignment['users']); echo '</pre>';
...
function variatives_enroll_report($filter) {
    global $DB, $CFG;
    ...
    if (isset($filter['varblockid'])) {
        if(is_array($filter['varblockid']) && count($filter['varblockid'])>0){
            $where[] = " var_block.id IN(".join(',',$filter['varblockid']).") ";
            //$param['varblockid'] = $filter['varblockid'];
            $assignment->load_best_from_queue($filter['varblockid']);
        }elseif($filter['varblockid']>0){
            $where[] = " var_block.id=:varblockid ";
            $param['varblockid'] = (int)$filter['varblockid'];
            $assignment->load_best_from_queue([$filter['varblockid']]);
    ...
    for($i=count($list)-1; $i>=0; $i--){
        $item=$list[$i];
        $key="{$item->userid}-{$item->varblockid}";
        if(!isset($users[$key])){
            unset($list[$i]);
            continue;
        }
        if(!isset($users[$key]['assignedcourses'][$item->courseid])){
            unset($list[$i]);
            continue;
        }
    }
...
function variatives_report_sql($varblockid) {
    ...
    $sql = [
        ...
        ['groupBy',  '`user`.id', 'var_group.id', 'var_subspecialityenroll.varsubspecialityid'],
        ['having'],
        ['orderBy',
...
function variatives_report_waiting($varblockid) {
    ...
    // var_dump($blockInfo); exit('001');
    // get block groups
    $blockGroups = variatives_blockgroup_list($varblockid);
    // echo "<hr><pre>";print_r($blockGroups); echo "</pre><hr>";
    // get list of students
    $sqlBuilder = new sqlBuilder();
    $sqlTree = variatives_report_sql($varblockid);
    // echo "<hr><pre>"; var_dump($sqlTree);  echo "</pre><hr>";
    //$tree2=$sqlBuilder->addTo($sqlTree, ['having'], 'rating_exists');
    // var_dump($tree2);
    ...
    // echo "<hr><pre>";print_r(count($rawList)); echo "</pre><hr>";
    // echo "<hr><pre>";print_r($rawList); echo "</pre><hr>";
    ...
    foreach ($blockGroups as $bg) {
        foreach ($rawList as $student) {
            ...
            //echo "<hr><pre> accept=$accept <br>bg / stud <br>";print_r($bg);print_r($student); echo "</pre><hr>";exit();
...
function variatives_user_courses($userid) {
    ...
    $sql = "
        ...
        WHERE cohort_members.userid=" . ( (int) $userid ) . "
              ...
              AND " . ( time() - 24 * 3600 ) . "<=var_block.varblocktimestampto
    ...
    $course_ids=[];
    foreach ($list as $li){
        $course_ids[]=(int)$li->courseid;
    }
    if(count($course_ids)>0){
        $course_ids=join(',',array_unique($course_ids));
        $sql="
            SELECT ctx.instanceid course_id, 
                    GROUP_CONCAT(CONCAT_WS(' ',' ',us.firstname, us.lastname)) AS tutorname
            FROM   {$CFG->prefix}context ctx,
                   {$CFG->prefix}role_assignments ras,
                   {$CFG->prefix}user us
            WHERE  ctx.id = ras.contextid
              AND  ras.roleid = 3
              AND  ras.userid = us.id
              AND ctx.instanceid IN  ($course_ids)
    ...
    foreach ($list as $li) {
        ...
        if(isset($tutors[$li->courseid])){
            $li->tutorname = $tutors[$li->courseid]->tutorname;
        }else{
            $li->tutorname='';
        }
...
class variatives_assignment {
    ...
    public $maxSearchTreeChildren=3;
    private static $courseRating;
    ...
    function initiate_load() {
        ...
        //echo "1<pre>";print_r($varblockcourserating);echo "</pre>";
        //echo "2<pre>";print_r($default_rating_keys);echo "</pre>";
        //echo "3<pre>";print_r($default_rating_vals);echo "</pre>";
        //exit();
        ...
        foreach ($list as $row) {
            ...
            $verbose=false;
            // if($key=='19332-1'){ $verbose=true; }
            
            if($row->varblockgroupnumcourses>0){
              if (!isset($this->assignment['users'][$key])) {
                ...
                if($verbose){
                   echo "<pre>"; print_r($this->assignment['users'][$key]); echo "</pre>";
                }
              }
              if($verbose){
                 echo " row->courseid={$row->courseid} row->varuserblockcourserating={$row->varuserblockcourserating};<br>";
              }
              if ($row->varuserblockcourserating) {
                $this->assignment['users'][$key]['rating'][$row->courseid] = $row->varuserblockcourserating;
              } else {
    ...
    private function getRandomRating($defaults) {
        ...
        $T = 4;//0.05 * $cnt; // temperature for annealing
        //echo "defaults:";print_r($defaults);echo("<br>");
        $arr = range(0, $cnt-1);
        ...
        for($i=0; $i<$cnt; $i+=1){
           //do{
             ...
             $i2=($i1+1)%$cnt;
             //rand(0,$cnt-1);
             // $diff=abs($i1-$i2);// + (- $defaults[$arr[$i1]] + $defaults[$arr[$i2]] );
             // $rnd = 0.001 * rand(1, 1000);
             // $edf = exp(-$diff / $T);
             // echo "$i1 $i2 :: $edf =?= $rnd<br>";
            //}while($rnd > $edf);
           list($arr[$i1],$arr[$i2]) = [$arr[$i2],$arr[$i1]];
           
           $diff=0;
           for($k=0; $k<$cnt;$k++){
              $diff+=abs($arr[$k]-$defaults[$k]);
           }
           //if($diff>$T*rand(1,10)*$cnt){
           if($diff>$T*rand(1,10)){
             break;
           }
           //print_r($arr);echo "<br>";
    ...
    public function assignment_allowed($user_block, $courseid) {
        ...
        foreach ($this->assignment['users'][$user_block]['assignedcourses'] as $course) {
            //if($user_block==$spy){
            //   echo("<hr> {$course} {$this->assignment['courses'][$course]['varblockcoursegroup'][$varblockid]} == {$varblockcoursegroup}<hr>");
            //}
            if ($this->assignment['courses'][$course]['varblockcoursegroup'][$varblockid] == $varblockcoursegroup) {
    ...
    public function toqueue($assignment) {
        ...
        $varassignmentqueuedata = json_encode($assignment);
        
        $counter = [];
        foreach ($assignment['courses'] as $courseid => $courseInfo) {
            $counter[$courseid] = array_values($courseInfo['users']);
            sort($counter[$courseid]);
        }
        ksort($counter);
        $hash = md5(json_encode($counter));
        //echo json_encode($counter).'<hr>';
        $sql = "SELECT id FROM {$CFG->prefix}var_assignmentqueue WHERE hashsum='$hash'";
        //echo "$sql <hr>";//exit('+++');
        $list = $DB->get_records_sql($sql);
        //$list=false;
        //print_r($list);
        if($list){
           echo "Skip duplicate ...<br><br>";
           return;
        }
        $info = new stdClass();
        sort($this->varblockids);
        ...
        $info->varassignmentqueuedata = $varassignmentqueuedata;
        $info->varassignmentqueueobjectivefunction = null;
        $info->hashsum = $hash;
        //exit();
        $res=$DB->insert_record('var_assignmentqueue', $info, $returnid = true);
    ...
    public function load_next_from_queue($varblockids) {
        ...
        $sql = "SELECT id
              ...
              ";
        //echo $sql;
        $list = array_values($DB->get_records_sql($sql, $params = null, $limitfrom = 0, $limitnum = 25));
        shuffle($list);
        //print_r($list);exit('%%%');
    ...
    public function count_queue($varblockids) {
        global $DB, $CFG;
        $sql = "SELECT SUM(varassignmentqueuestatus='undefined') as waiting, 
                       count(*) as total,
                       SUM(varassignmentqueuestatus='variant') as variants
    ...
    public function processassignment() {
        ...
        if ($this->assignment) {
            ...
            if (count($number_of_students) == 0) {
                ...
                $info->varassignmentqueuecorrelation=self::getcorrelation($this->assignment);
                $DB->update_record('var_assignmentqueue', $info);
            } else {
                //echo '<pre>'; print_r($number_of_students); echo '</pre>';  exit();

                echo '<pre>'.count($number_of_students).' courses have not enough students, try exclude them</pre>';
                //echo '<pre> courses:<br>';  print_r($number_of_students); echo '</pre>';
                // get min allowed number of students
                $counts =  array_unique(array_values($number_of_students));
                sort($counts);
                $cnt=count($counts);
                if($cnt>$this->maxSearchTreeChildren){
                    $cnt = $this->maxSearchTreeChildren;
                }
                $minStudents=$counts[$cnt-1];

                // echo '<pre> $counts:<br>';  print_r($counts); echo '</pre>';  // exit('<hr>003');
                // echo '<pre> $counts:<br>';  print_r($minStudents); echo '</pre>';  // exit('<hr>004');
                
                $courseids = array_keys($number_of_students);
                // echo '<pre> $courseids:<br>';  print_r($courseids); echo '</pre>';   // exit('<hr>005');
                foreach ($courseids as $courseid){
                    // echo "{$number_of_students[$courseid]} > $minStudents ;<br>";
                    if($number_of_students[$courseid]>$minStudents){
                        // echo " do not exclude ;<br>";
                        unset($number_of_students[$courseid]);
                    }
                }
                echo '<pre>Candidates to exclude:<br>';  print_r($number_of_students); echo '</pre>';  // exit('<hr>003');
                ...
                foreach ($courseids as $courseid) {
                    ...
                    print_r(['courseid'=>$courseInfo['courseid'],'course_fullname'=>$courseInfo['course_fullname'],'varblockminstudents'=>$courseInfo['varblockminstudents'],]);
                    echo '</pre>';
                    ...
                    if ($all_students_moved) {
                        //echo '<pre>'; print_r($newassignment); echo '</pre>';//  exit();
                        // echo '<pre>'; print_r(join(',', array_keys($newassignment->assignment['courses']))); echo '</pre>'; //  exit();
                ...
                $info = new stdClass();
                $info->id = $this->assignmentid;
                $info->varassignmentqueuestatus = 'invalid';
                // $info->varassignmentqueueobjectivefunction = $this->objectivefunction();
                // $info->varassignmentqueuecorrelation=self::getcorrelation($this->assignment);
                $DB->update_record('var_assignmentqueue', $info);
            ...
            // assignment with 'undefined' status not found
            return false;
        }
    }

    public static function getcorrelation($assignment){
        global $CFG,$DB;

        if(!isset(self::$courseRating)){
            $varblockids=[];
            foreach ($assignment['blocks'] as $block) {
               $varblockids[]=$block['id'];
            }

            $sql = "SELECT ubc.courseid, c.fullname, AVG(ubc.varuserblockcourserating) avr, COUNT(*) AS nvr
                    FROM {$CFG->prefix}var_userblockcourse AS ubc
                         INNER JOIN {$CFG->prefix}course c ON c.id=ubc.courseid
                    WHERE ubc.varblockid  IN(" . join(',', $varblockids) . ")
                    GROUP BY ubc.courseid
                    ORDER BY avr
            ";
            // print_r(Array('waiting'=> $list[0]->waiting, 'total'=> $list[0]->total )); exit('rrr');
            $list=$DB->get_records_sql($sql);
            ...
            self::$courseRating=[];
            foreach($list as $n){
               self::$courseRating[$n->courseid]=0;
               foreach($list as $n1){
                 if( $n1->avr <= $n->avr ){
                    self::$courseRating[$n->courseid]++;
                 }
               }
            }
            //echo "<pre>default rating ";print_r(self::$courseRating);echo '</pre>';//exit();
        }
        $counter = [];
        foreach(self::$courseRating  as $courseid=>$rank){
           $counter[$courseid]=0;
        }
        foreach ($assignment['courses'] as $courseid => $courseInfo) {
            $counter[$courseid] = count($courseInfo['users']);
        ...
        arsort($counter);
        //echo "<pre>counter rating ";print_r($counter);echo '</pre>';//exit();
        $ranks=[];
        foreach($counter as $courseid => $n){
           $ranks[$courseid]=0;
           foreach($counter as $c1 => $n1){
              if($n1>=$n){
                  $ranks[$courseid]++;
              }
           }
        ...
        $sum=0;
        //echo "<pre>real rating ";print_r($ranks);echo '</pre>';//exit();
        $n=count(self::$courseRating);
        foreach(self::$courseRating  as $courseid=>$rank){
           $d=$rank-$ranks[$courseid];
           $sum+=$d*$d;
        }
        $correlation=1-6*$sum/$n/($n*$n-1);
        //echo "correlation=$correlation<br>";exit('***9');
        return $correlation;
    }
    ...
    public function load_best_from_queue($varblockids) {
        ...
        $sql = "SELECT *
              ...
              ORDER BY varassignmentqueueobjectivefunction ASC,
                       varassignmentqueuecorrelation DESC";
        // echo $sql;
        $list = array_values($DB->get_records_sql($sql, $params = null, $limitfrom = 0, $limitnum = 1));
        // print_r($list);
